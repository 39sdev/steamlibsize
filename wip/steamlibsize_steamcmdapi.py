...
def get_app_data(appid):
    ...
    app_info = requests.get("https://api.steamcmd.net/v1/info/740")
    ...
    with open(f"appcache/{appid}.json", "w") as output_app_file:
        if isinstance(recieved_app_data, str):
            output_app_file.write(recieved_app_data)
            print(f"appinfo dumper: written {appid}")
        else:
            print(f"appinfo dumper: Error getting info about {appid}")
